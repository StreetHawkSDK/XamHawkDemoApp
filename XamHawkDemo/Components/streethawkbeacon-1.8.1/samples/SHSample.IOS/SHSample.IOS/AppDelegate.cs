...
namespace SHSample.IOS
{
	...
	public partial class AppDelegate : UIApplicationDelegate
	{
		...
		{
			//location, geofence and beacon's enable status should be same, use either one of them. 
			SHBeacon.instance().isDefaultLocationServiceEnabled = false;

			//streethawk register install sample.
			SHApp.instance ().appKey = "XSample";
			...
			//streethawk callback when enter exit beacon.
			SHBeacon.instance ().notifyBeaconDetectCallback = delegate (NSDictionary beacon) 
			{
				InvokeOnMainThread ( () => {
					UIAlertView alert = new UIAlertView ("Beacon enter or exit: ", beacon.ToString(), null, "OK", null);
					alert.Show ();
				});
			};
